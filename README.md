...
## Technical Aspects
The CSV file contains 10.511 records in rows and 23 columns. For a correct download click on the PORTRAITS-metadata.csv and then right-click on 'Download' selecting 'Save link as'. In order to correctly display the CSV file and the registered records, it is suggested to use Libre or Open Office. The CSV file contains the following metadata:
